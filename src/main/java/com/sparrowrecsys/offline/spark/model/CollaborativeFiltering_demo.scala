...
object CollaborativeFiltering_demo {
  def main(args: Array[String]): Unit = {

    val conf = new SparkConf()
      .setMaster("local")
      .setAppName("collaborateFiltering")
    ...
    val als = new ALS().setMaxIter(5).setRegParam(0.01).setUserCol("userIdInt").setItemCol("movieIdInt").setRatingCol("ratingFloat")
    ...
    val evaluator = new RegressionEvaluator().setMetricName("rmse").setLabelCol("ratingFloat").setPredictionCol("prediction")
    ...
    // Generate top 10 movie recommendation for a specified set of users
    ...
    val paramGrid = new ParamGridBuilder().addGrid(als.regParam, Array(0.01)).build()
