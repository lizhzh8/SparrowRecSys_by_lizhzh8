...
object FeatureEngineering_demo {
  // one hot encoding example function
  //samples movie samples dataframe
  ...
  def ratingFeatures(samples: DataFrame): Unit = {
    ...
    val movieFeatures = samples.groupBy(col("movieId")).agg(count(lit(1)).as("ratingCount"),
      avg(col("rating")).as("avgRating"), variance(col("rating")).as("ratingVar")).withColumn("avgRatingVec", double2vec(col("avgRating")))
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("local").setAppName("featureEngineering_demo").set("spark.submit,deployMode", "client")
