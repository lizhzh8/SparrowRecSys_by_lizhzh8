...
// DataManger is an utility class,takes charge of all data loading logic.
public class DataManager_demo {
    // singleton instance
    private static volatile DataManager_demo instance;
    ...
    public static DataManager_demo getInstance() {
        if (null == instance) {
            synchronized (DataManager_demo.class) {
                if (null == instance) {
                    instance = new DataManager_demo();
                }
            }
        }
        return instance;
    ...
    private void loadMovieData(String movieDataPath) throws Exception {
        ...
        System.out.println("loading movine data completed." + this.movieMap.size() + " movies in total.");
    }

    // load moive embedding
    private void loadMovieEmb(String movieEmbPath, String embKey) throws Exception {
        if (Config.EMB_DATA_SOURCE.equals(Config.DATA_SOURCE_FILE)) {
            System.out.println("Loading movie embeding from " + movieEmbPath + " ...");
            int validEmbCount = 0;
    ...
    // load movie featues
    private void loadMovieFeatures(String moiveFeaturesPrefix) throws Exception {
        System.out.println("Loading movie features from Redis ...");
        Set<String> movieFeaturesKeys = RedisClient.getInstance().keys(moiveFeaturesPrefix + "*");
    ...
    private void loadUserEmb(String userEmbPath, String embKey) throws Exception {
        if (Config.EMB_DATA_SOURCE.equals(Config.DATA_SOURCE_FILE)) {
            System.out.println("Loading user embeding from " + userEmbPath + " ...");
            int validEmbCount = 0;
            ...
            while (scanner.hasNextLine()) {
                ...
                if (linkData.length == 3) {
                    ...
                    if (null != movie) {
                        ...
                        movie.setImdbId(linkData[2].trim());
                    }
                }
            }
        }
        ...
        try (Scanner scanner = new Scanner(new File(ratingDataPath))) {
            while (scanner.hasNextLine()) {
                ...
                if (linkData.length == 4) {
                    ...
                    rating.setScore(Float.parseFloat(linkData[3]));
    ...
    // get movies by genre,and order the movies by sorted method
    public List<Movie> getMovieByGenre(String genre, int size, String sortBy) {
        if (null != genre) {
